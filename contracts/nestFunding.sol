...
contract NestCrowdFunding {
    ...
    function createCampagin(address _campaginOwner, string memory _title, string memory _description, uint256 _target, uint256 _deadline, string memory _images) public returns (uint256) {

        Campagin storage campagin= campagins[numOfCampagins];
        //deadline checkpoint
        require(campagin.deadlineProject< block.timestamp, "the deadline should be a target date for future of minimum 12 hours! ");

        campagin.userOwner= _campaginOwner;
        campagin.title= _title;
        campagin.description= _description;
        campagin.targetAmmount= _target;
    ...
    function getContributors(uint256 _userID)public view returns(address[] memory, uint256[] memory){
